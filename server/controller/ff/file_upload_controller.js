...
const uploadChangeDB = async (req, res, next) => {
		...
		if (folders.length > 0) {
			for (let i = 0; i < folders.length; i++) {
				...
				if (chkDir.length === 0) {
					...
					await changeFolderDeleleStatus(
						0,
						chkDir[0].id,
						nowTime
					);
    ...
    console.error(`uploadChangeDB: ${e}`);
...
const uploadCommitDB = async (req, res) => {
  ...
  try {
    ...
    console.log("commit.serverStatus: ", commit.serverStatus);
    ...
    console.error(`uploadCommitDB: ${e}`);
    return res.status(500).json({ error: "Something broken" });
  }
};
