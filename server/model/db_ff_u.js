...
const updateDeletedFile = async (del_status, token, file_id, file_size, time) => {
  ...
  try {
    ...
    const ff = await conn.query(`
      UPDATE ff 
      SET is_delete = ?, upd_status = "pending", token = ?, updated_at = ? 
      WHERE id = ? 
    `, [del_status, token, time, file_id]);
...
const commitMetadata = async(upd_status, token) => {
  const [row] = await pool.query(`
    UPDATE ff SET upd_status = ?, token = NULL WHERE token = ?
  `, [upd_status, token]);
  return row;
};
...
const restoreFileToPrev = async(token, file_id, version, time, user_id) => {
  ...
  try {
    ...
    const ff = await conn.query(`
      UPDATE ff SET upd_status = "pending", token = ?, updated_at = ? WHERE id = ? 
    `, [token, time, file_id]);
...
const restoreDeletedFile = async(token, file_id, time, user_id) => {
  ...
  try {
    ...
    const ff = await conn.query(`
      UPDATE ff 
      SET upd_status = "pending", token = ?, updated_at = ?, is_delete = 0 
      WHERE id = ? 
    `, [token, time, file_id]);
